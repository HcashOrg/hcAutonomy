...
func usage() {
	fmt.Fprintf(os.Stderr, "usage: hcAutonomyvoter [flags] <action> [arguments]\n")
...
func ProvidePrivPassphrase() ([]byte, error) {
	...
	for {
		...
		//pass, err := terminal.ReadPassword(111111)
		if err != nil {
			return nil, err
		}
		fmt.Print("\n")
...
func verifyMessage(address, message, signature string) (bool, error) {
	...
	a, err := hcutil.NewAddressSecpPubKey(serializedPK, activeNetParams.Params)
	if err != nil {
...
func (c *ctx) getCSRF() (*v1.VersionReply, error) {
	...
	fullRoute := c.cfg.HcAutonomyWWW + v1.HcAutonomyWWWAPIRoute + v1.RouteVersion
...
func (c *ctx) makeRequest(method, route string, b interface{}) ([]byte, error) {
	...
	fullRoute := c.cfg.HcAutonomyWWW + v1.HcAutonomyWWWAPIRoute + route +
		queryParams
...
// autonomyvoter while it is in the process of casting votes.
