...
import (
	...
	hcdataapi "github.com/HcashOrg/hcexplorer/hcdataapi"
...
const (
	HcashOrgPluginIdentity = "fullidentity"
	HcashOrgPluginJournals = "journals"
...
type CastVoteJournal struct {
	CastVote HcashOrgplugin.CastVote `json:"castvote"` // Client side vote
...
var (
	HcashOrgPluginSettings map[string]string             // [key]setting
	HcashOrgPluginHooks    map[string]func(string) error // [key]func(token) error

	// cached values, requires lock
	// XXX why is this a pointer? Convert if possible after investigating
	HcashOrgPluginVoteCache = make(map[string]*HcashOrgplugin.StartVote) // [token]startvote
	...
	pluginDataDir = filepath.Join("plugins", "HcashOrg")

	// Individual votes cache
	//HcashOrgPluginVotesCache = make(map[string]map[string]HcashOrgplugin.CastVote) // [token][ticket]castvote
	HcashOrgPluginVotesCache = make(map[string]map[string]struct{})

	HcashOrgPluginCommentsCache      = make(map[string]map[string]HcashOrgplugin.Comment) // [token][commentid]comment
	HcashOrgPluginCommentsLikesCache = make(map[string][]HcashOrgplugin.LikeComment)      // [token]LikeComment
...
func getHcPlugin(testnet bool) backend.Plugin {
	HcashOrgPlugin := backend.Plugin{
		ID:       HcashOrgplugin.ID,
		Version:  HcashOrgplugin.Version,
		Settings: []backend.PluginSetting{},
	}

	if testnet {
		HcashOrgPlugin.Settings = append(HcashOrgPlugin.Settings,
			backend.PluginSetting{
				Key:   "hcdata",
				Value: "https://testnet-data.h.cash/",
			},
		)
	} else {
		HcashOrgPlugin.Settings = append(HcashOrgPlugin.Settings,
			backend.PluginSetting{
				Key:   "hcdata",
				Value: "https://data.h.cash/",
			})
	}

	// Initialize hooks
	HcashOrgPluginHooks = make(map[string]func(string) error)

	// Initialize settings map
	HcashOrgPluginSettings = make(map[string]string)
	for _, v := range HcashOrgPlugin.Settings {
		HcashOrgPluginSettings[v.Key] = v.Value
	}
	return HcashOrgPlugin
}

// initHcPlugin is called externally to run initial procedures
// such as replaying journals
func (g *gitBackEnd) initHcPluginJournals() error {
	log.Infof("initHcPlugin")

	// check if backend journal is intialized
	if g.journal == nil {
		return fmt.Errorf("initHcPlugin backend journal isn't initialized")
	}

	err := g.replayAllJournals()
	if err != nil {
		log.Infof("initHcPlugin replay all journals %v", err)
	}
	return nil
}
...
//SetHcPluginSetting removes a setting if the value is "" and adds a setting otherwise.
func setHcPluginSetting(key, value string) {
	if value == "" {
		delete(HcashOrgPluginSettings, key)
		return
	}

	HcashOrgPluginSettings[key] = value
}

func setHcPluginHook(name string, f func(string) error) {
	HcashOrgPluginHooks[name] = f
}
...
// Copied from https://github.com/HcashOrg/hcd/blob/0fc55252f912756c23e641839b1001c21442c38a/rpcserver.go#L5605
func (g *gitBackEnd) verifyMessage(address, message, signature string) (bool, error) {
	...
	a, err := hcutil.NewAddressSecpPubKey(serializedPK, g.activeNetParams)
	if err != nil {
	...
	address1:=a.EncodeAddress()
	return address1 == address, nil
}

func bestBlock() (*hcdataapi.BlockDataBasic, error) {
	url := HcashOrgPluginSettings["hcdata"] + "api/block/best"
	...
	var bdb hcdataapi.BlockDataBasic
...
func block(block uint32) (*hcdataapi.BlockDataBasic, error) {
	h := strconv.FormatUint(uint64(block), 10)
	url := HcashOrgPluginSettings["hcdata"] + "api/block/" + h
...
func snapshot(hash string) ([]string, error) {
	url := HcashOrgPluginSettings["hcdata"] + "api/stake/pool/b/" + hash +
		"/full?sort=true"
	log.Debugf("connecting to %v", url)
	tr := &http.Transport{
        TLSClientConfig:    &tls.Config{InsecureSkipVerify: true},
...
func batchTransactions(hashes []string) ([]hcdataapi.TrimmedTx, error) {
	// Request body is hcdataapi.Txns marshalled to JSON
	reqBody, err := json.Marshal(hcdataapi.Txns{
		Transactions: hashes,
	})
	if err != nil {
		return nil, err
	...
	url := HcashOrgPluginSettings["hcdata"] + "api/txs/trimmed"
	log.Debugf("connecting to %v", url)
//	tr := &http.Transport{
  //      TLSClientConfig:    &tls.Config{InsecureSkipVerify: true},
    //	}
   //	client := &http.Client{Transport: tr}
	r, err2 := http.Post(url, "application/json; charset=utf-8",
...
func largestCommitmentAddresses(hashes []string) ([]largestCommitmentResult, error) {
	// Batch request all of the transaction info from hcdata.
...
// HcashOrgPluginPostEdit called after and edit is complete but before commit.
func (g *gitBackEnd) HcashOrgPluginPostEdit(token string) error {
	log.Tracef("HcashOrgPluginPostEdit: %v", token)
	...
	if destination == "" {
		log.Tracef("HcashOrgPluginPostEdit: nothing to do %v", token)
		return nil
	}
...
// flushCommentflushes comments journal to HcashOrg plugin directory in
...
// flushVotes flushes votes journal to HcashOrg plugin directory in git. It
...
func (g *gitBackEnd) pluginNewComment(payload string) (string, error) {
	...
	// Decode comment
	comment, err := HcashOrgplugin.DecodeNewComment([]byte(payload))
	if err != nil {
	...
	blob, err := HcashOrgplugin.EncodeComment(c)
	if err != nil {
	...
	// Remove from cash.
	if _, ok := HcashOrgPluginCommentsCache[c.Token]; !ok {
		HcashOrgPluginCommentsCache[c.Token] =
			make(map[string]HcashOrgplugin.Comment)
	} else {
		// Sanity
		log.Errorf("comment should not have existed.")
	}
	HcashOrgPluginCommentsCache[c.Token][c.CommentID] = c
	g.Unlock()

	// Encode reply
	ncr := HcashOrgplugin.NewCommentReply{
		Comment: c,
	}
	ncrb, err := HcashOrgplugin.EncodeNewCommentReply(ncr)
	if err != nil {
...
func replyLikeCommentReplyError(failure error) (string, error) {
	lcr := HcashOrgplugin.LikeCommentReply{
		Error: failure.Error(),
	}
	lcrb, err := HcashOrgplugin.EncodeLikeCommentReply(lcr)
	if err != nil {
...
func (g *gitBackEnd) pluginLikeComment(payload string) (string, error) {
	...
	// Decode comment
	like, err := HcashOrgplugin.DecodeLikeComment([]byte(payload))
	if err != nil {
	...
	// Verify cache
	c, ok := HcashOrgPluginCommentsCache[like.Token][like.CommentID]
	if !ok {
		g.Unlock()
	...
	cc := HcashOrgPluginCommentsLikesCache[like.Token]

	// Update cache
	HcashOrgPluginCommentsLikesCache[like.Token] = append(cc, *like)
	g.Unlock()
	...
	unwind := func() {
		g.Lock()
		HcashOrgPluginCommentsLikesCache[like.Token] = cc
		g.Unlock()
	}

	// Create Journal entry
	lc := HcashOrgplugin.LikeComment{
		Token:     like.Token,
	...
	blob, err := HcashOrgplugin.EncodeLikeComment(lc)
	if err != nil {
		unwind()
...
func (g *gitBackEnd) pluginCensorComment(payload string) (string, error) {
	...
	censor, err := HcashOrgplugin.DecodeCensorComment([]byte(payload))
	if err != nil {
	...
	// Verify cache
	_, ok = HcashOrgPluginCommentsCache[censor.Token]
	if !ok {
		g.Unlock()
	...
	c, ok := HcashOrgPluginCommentsCache[censor.Token][censor.CommentID]
	if !ok {
		g.Unlock()
		return "", fmt.Errorf("comment not found %v:%v",
	...
	c.Comment = ""
	c.Censored = true
	HcashOrgPluginCommentsCache[censor.Token][censor.CommentID] = c
	...
	unwind := func() {
		g.Lock()
		HcashOrgPluginCommentsCache[censor.Token][censor.CommentID] = oc
		g.Unlock()
	}

	// Create Journal entry
	cc := HcashOrgplugin.CensorComment{
		Token:     censor.Token,
	...
	blob, err := HcashOrgplugin.EncodeCensorComment(cc)
	if err != nil {
		unwind()
	...
	// Encode reply
	ccr := HcashOrgplugin.CensorCommentReply{
		Receipt: cc.Receipt,
	}
	ccrb, err := HcashOrgplugin.EncodeCensorCommentReply(ccr)
	if err != nil {
		unwind()
...
// returned as a HcashOrgplugin reply. If the comment map is nil it returns a
// valid empty reply structure.
func encodeGetCommentsReply(cm map[string]HcashOrgplugin.Comment) (string, error) {
	if cm == nil {
		cm = make(map[string]HcashOrgplugin.Comment)
	}

	// Encode reply
	gcr := HcashOrgplugin.GetCommentsReply{
		Comments: make([]HcashOrgplugin.Comment, 0, len(cm)),
	}
	for _, v := range cm {
		gcr.Comments = append(gcr.Comments, v)
	}

	gcrb, err := HcashOrgplugin.EncodeGetCommentsReply(gcr)
	if err != nil {
...
// this function can be called WITHOUT the lock held
func (g *gitBackEnd) replayComments(token string) (map[string]HcashOrgplugin.Comment, error) {
	...
	comments := make(map[string]HcashOrgplugin.Comment)
	commentsLikes := make([]HcashOrgplugin.LikeComment, 0, 1024)
	...
	for {
		err = g.journal.Replay(cfilename, func(s string) error {
			...
			case journalActionDel:
				var cc HcashOrgplugin.CensorComment
				err = d.Decode(&cc)
				if err != nil {
			...
			case journalActionAddLike:
				var lc HcashOrgplugin.LikeComment
				err = d.Decode(&lc)
				if err != nil {
	...
	g.Lock()
	HcashOrgPluginCommentsCache[token] = comments
	HcashOrgPluginCommentsLikesCache[token] = commentsLikes
	g.Unlock()
...
func (g *gitBackEnd) pluginGetProposalCommentsLikes(payload string) (string, error) {
	var gpclr HcashOrgplugin.GetProposalCommentsLikesReply

	gpcl, err := HcashOrgplugin.DecodeGetProposalCommentsLikes([]byte(payload))
	if err != nil {
		return "", fmt.Errorf("DecodeGetProposalCommentsLikes: %v", err)
	}

	g.Lock()
	gpclr.CommentsLikes = HcashOrgPluginCommentsLikesCache[gpcl.Token]
	g.Unlock()

	egpclr, err := HcashOrgplugin.EncodeGetProposalCommentsLikesReply(gpclr)
	if err != nil {
...
func (g *gitBackEnd) pluginGetComments(payload string) (string, error) {
	...
	// Decode comment
	gc, err := HcashOrgplugin.DecodeGetComments([]byte(payload))
	if err != nil {
		return "", fmt.Errorf("DecodeGetComments: %v", err)
	}

	g.Lock()
	comments := HcashOrgPluginCommentsCache[gc.Token]
	g.Unlock()
...
func (g *gitBackEnd) pluginAuthorizeVote(payload string) (string, error) {
	...
	authorize, err := HcashOrgplugin.DecodeAuthorizeVote([]byte(payload))
	if err != nil {
	...
	av := HcashOrgplugin.AuthorizeVote{
		Version:   HcashOrgplugin.VersionAuthorizeVote,
		Receipt:   receipt,
		Timestamp: time.Now().Unix(),
	...
	avb, err := HcashOrgplugin.EncodeAuthorizeVote(av)
	if err != nil {
	...
	_, err = os.Stat(pijoin(joinLatest(g.vetted, token),
		fmt.Sprintf("%02v%v", HcashOrgplugin.MDStreamVoteBits,
	...
	err = g._updateVettedMetadata(tokenb, nil, []backend.MetadataStream{
		{
			ID:      HcashOrgplugin.MDStreamAuthorizeVote,
			Payload: string(avb),
		},
	})
	if err != nil {
...
func (g *gitBackEnd) pluginStartVote(payload string) (string, error) {
	vote, err := HcashOrgplugin.DecodeStartVote([]byte(payload))
	if err != nil {
	...
	if vote.Vote.Duration < HcashOrgplugin.VoteDurationMin ||
		vote.Vote.Duration > HcashOrgplugin.VoteDurationMax {
		// XXX return a user error instead of an internal error
		return "", fmt.Errorf("invalid duration: %v (%v - %v)",
			vote.Vote.Duration, HcashOrgplugin.VoteDurationMin,
			HcashOrgplugin.VoteDurationMax)
	}

	svr := HcashOrgplugin.StartVoteReply{
		Version: HcashOrgplugin.VersionStartVoteReply,
	...
	svrb, err := HcashOrgplugin.EncodeStartVoteReply(svr)
	if err != nil {
		return "", fmt.Errorf("EncodeStartVoteReply: %v", err)
	}

	// Add version to on disk structure
	vote.Version = HcashOrgplugin.VersionStartVote
	voteb, err := HcashOrgplugin.EncodeStartVote(*vote)
	if err != nil {
	...
	_, err1 := os.Stat(pijoin(joinLatest(g.vetted, token),
		fmt.Sprintf("%02v%v", HcashOrgplugin.MDStreamAuthorizeVote,
			defaultMDFilenameSuffix)))
	_, err2 := os.Stat(pijoin(joinLatest(g.vetted, token),
		fmt.Sprintf("%02v%v", HcashOrgplugin.MDStreamVoteBits,
			defaultMDFilenameSuffix)))
	_, err3 := os.Stat(pijoin(joinLatest(g.vetted, token),
		fmt.Sprintf("%02v%v", HcashOrgplugin.MDStreamVoteSnapshot,
			defaultMDFilenameSuffix)))
	...
	av, err := HcashOrgplugin.DecodeAuthorizeVote(b)
	if err != nil {
		return "", fmt.Errorf("DecodeAuthorizeVote: %v", err)
	}
	...
	err = g._updateVettedMetadata(tokenB, nil, []backend.MetadataStream{
		{
			ID:      HcashOrgplugin.MDStreamVoteBits,
			Payload: string(voteb),
		},
		{
			ID:      HcashOrgplugin.MDStreamVoteSnapshot,
			Payload: string(svrb),
		}})
	if err != nil {
		return "", fmt.Errorf("_updateVettedMetadata: %v", err)
...
// exists.
func _validateVoteBit(vote HcashOrgplugin.Vote, bit uint64) error {
...
func (g *gitBackEnd) validateVoteBit(token, bit string) error {
	...
	sv, ok := HcashOrgPluginVoteCache[token]
	if ok {
	...
	f, err := os.Open(mdFilename(g.vetted, token,
		HcashOrgplugin.MDStreamVoteBits))
	if err != nil {
		return err
	}
	defer f.Close()
	...
	HcashOrgPluginVoteCache[token] = sv
	...
	for {
		err = g.journal.Replay(bfilename, func(s string) error {
			...
			case journalActionAdd:
				...
				if _, ok := HcashOrgPluginVotesCache[token]; !ok {
					// Create map to track tickets
					HcashOrgPluginVotesCache[token] = make(map[string]struct{})
				}
				// See if we have a duplicate vote
				if _, ok := HcashOrgPluginVotesCache[token][ticket]; ok {
					log.Errorf("duplicate cast vote %v %v",
						token, ticket)
				}
				// All good, record vote in cache
				HcashOrgPluginVotesCache[token][ticket] = struct{}{}
...
//
// Functions must be called WITH the lock held.
func (g *gitBackEnd) voteExists(v HcashOrgplugin.CastVote) (bool, error) {
	// Check if journal exists, use fast path
	_, ok := HcashOrgPluginVotesCache[v.Token][v.Ticket]
	return ok, nil
}
...
func (g *gitBackEnd) pluginBallot(payload string) (string, error) {
	...
	// Decode ballot
	ballot, err := HcashOrgplugin.DecodeBallot([]byte(payload))
	if err != nil {
	...
	br := HcashOrgplugin.BallotReply{
		Receipts: make([]HcashOrgplugin.CastVoteReply, len(ballot.Votes)),
	}
	for k, v := range ballot.Votes {
		...
		// Add to cache
		g.Lock()
		if _, ok := HcashOrgPluginVotesCache[v.Token]; !ok {
			HcashOrgPluginVotesCache[v.Token] = make(map[string]struct{})
		}
		HcashOrgPluginVotesCache[v.Token][v.Ticket] = struct{}{}
		g.Unlock()
	...
	// Encode reply
	brb, err := HcashOrgplugin.EncodeBallotReply(br)
	if err != nil {
...
func (g *gitBackEnd) tallyVotes(token string) ([]HcashOrgplugin.CastVote, error) {
	// Do some cheap things before expensive calls
	...
	if err != nil {
		...
		return []HcashOrgplugin.CastVote{}, nil
	}
	defer func() {
		err = g.journal.Close(bfilename)
		if err != nil {
	...
	cv := make([]HcashOrgplugin.CastVote, 0, 41000)
	for {
		err = g.journal.Replay(bfilename, func(s string) error {
			ss := bytes.NewReader([]byte(s))
			d := json.NewDecoder(ss)
...
func (g *gitBackEnd) pluginProposalVotes(payload string) (string, error) {
	...
	vote, err := HcashOrgplugin.DecodeVoteResults([]byte(payload))
	if err != nil {
	...
	// Prepare reply
	var vrr HcashOrgplugin.VoteResultsReply
...
nodata:
	reply, err := HcashOrgplugin.EncodeVoteResultsReply(vrr)
	if err != nil {
