...
// verifyAnchor asks hctime if an anchor has been verified and returns a TX if
// it has.
func (g *gitBackEnd) verifyAnchor(digest string) (*v1.VerifyDigest, error) {
	...
	if g.test {
		...
		// Call hctime
		vr, err = util.Verify(g.hctimeHost, []string{digest})
		if err != nil {
			return nil, err
		}
	}
...
func (g *gitBackEnd) _updateRecord(commit bool, id string, mdAppend, mdOverwrite []backend.MetadataStream, fa []file, filesDel []string) error {
	...
	avFilename := fmt.Sprintf("%02v%v", HcashOrgplugin.MDStreamAuthorizeVote,
		defaultMDFilenameSuffix)
	...
	// Call plugin hooks
	f, ok := HcashOrgPluginHooks[PluginPostHookEdit]
	if ok {
...
// hctime.  This is an expensive operation and should not be run during
// runtime.
//
...
func (g *gitBackEnd) fsck(path string) error {
	...
	var seenAnchor bool
	// gitDigests is an index of all git digests to verify with hctime
	gitDigests := make(map[string]struct{})
	// confirmedAnchors keeps track of anchors that were timestamped with hctime but not verified,
	...
	for _, v := range out {
		if regexAnchorConfirmation.MatchString(v) {
			...
			// If we didn't, save the anchor key to verify with hctime later
	...
	log.Infof("fsck: hctime verification started")
	...
	vr, err := util.Verify(g.hctimeHost, digests)
	if err != nil {
		return err
	}
	...
	for _, v := range vr.Digests {
		if v.Result != v1.ResultOK {
			fail = true
			log.Errorf("hctime error: %v %v %v", v.Digest,
				v.Result, v1.Result[v.Result])
		}
	}
	if fail {
		return fmt.Errorf("hctime fsck failed")
	}
...
func (g *gitBackEnd) Plugin(command, payload string) (string, string, error) {
	...
	switch command {
	case HcashOrgplugin.CmdAuthorizeVote:
		payload, err := g.pluginAuthorizeVote(payload)
		return HcashOrgplugin.CmdAuthorizeVote, payload, err
	case HcashOrgplugin.CmdStartVote:
		payload, err := g.pluginStartVote(payload)
		return HcashOrgplugin.CmdStartVote, payload, err
	case HcashOrgplugin.CmdBallot:
		payload, err := g.pluginBallot(payload)
		return HcashOrgplugin.CmdBallot, payload, err
	case HcashOrgplugin.CmdProposalVotes:
		payload, err := g.pluginProposalVotes(payload)
		return HcashOrgplugin.CmdProposalVotes, payload, err
	case HcashOrgplugin.CmdBestBlock:
		payload, err := g.pluginBestBlock()
		return HcashOrgplugin.CmdBestBlock, payload, err
	case HcashOrgplugin.CmdNewComment:
		payload, err := g.pluginNewComment(payload)
		return HcashOrgplugin.CmdNewComment, payload, err
	case HcashOrgplugin.CmdLikeComment:
		payload, err := g.pluginLikeComment(payload)
		return HcashOrgplugin.CmdLikeComment, payload, err
	case HcashOrgplugin.CmdCensorComment:
		payload, err := g.pluginCensorComment(payload)
		return HcashOrgplugin.CmdCensorComment, payload, err
	case HcashOrgplugin.CmdGetComments:
		payload, err := g.pluginGetComments(payload)
		return HcashOrgplugin.CmdGetComments, payload, err
	case HcashOrgplugin.CmdProposalCommentsLikes:
		payload, err := g.pluginGetProposalCommentsLikes(payload)
		return HcashOrgplugin.CmdProposalCommentsLikes, payload, err
	}
...
func New(anp *chaincfg.Params, root string, hctimeHost string, gitPath string, id *identity.FullIdentity, gitTrace bool) (*gitBackEnd, error) {
	...
	g := &gitBackEnd{
		...
		gitPath:         gitPath,
		hctimeHost:     hctimeHost,
		gitTrace:        gitTrace,
		exit:            make(chan struct{}),
		checkAnchor:     make(chan struct{}),
		testAnchors:     make(map[string]bool),
		plugins:         []backend.Plugin{getHcPlugin(anp.Name != "mainnet")},
	}
	idJSON, err := id.Marshal()
	if err != nil {
		return nil, err
	}
	setHcPluginSetting(HcashOrgPluginIdentity, string(idJSON))
	setHcPluginSetting(HcashOrgPluginJournals, g.journals)
	setHcPluginHook(PluginPostHookEdit, g.HcashOrgPluginPostEdit)
	...
	err = g.cron.AddFunc(anchorSchedule, func() {
		// Flush journals
		g.HcashOrgPluginJournalFlusher()
	...
	// Message user
	log.Infof("Timestamp host: %v", g.hctimeHost)

	log.Infof("Running hctime fsck on vetted repository")
	err = g.fsck(g.vetted)
	if err != nil {
		// Log error but continue
		log.Errorf("fsck: hctime %v", err)
	}
