...
const (
	defaultConfigFilename   = "hcAutonomyd.conf"
	defaultDataDirname      = "data"
	defaultLogLevel         = "info"
	defaultLogDirname       = "logs"
	defaultLogFilename      = "hcAutonomyd.log"
...
var (
	defaultHomeDir       = hcutil.AppDataDir("hcAutonomyd", false)
...
type config struct {
	...
	HctimeHost string `long:"hctimehost" description:"hctime ip:port"`
	HctimeCert string `long:"hctimecert" description:"File containing the https certificate file for hctimehost"`
	Identity   string `long:"identity" description:"File containing the hcAutonomyd identity file"`
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.TestNet {
		var timeHost string
		if len(cfg.HctimeHost) == 0 {
			timeHost = v1.DefaultTestnetTimeHost
		} else {
			timeHost = cfg.HctimeHost
		}
		cfg.HctimeHost = util.NormalizeAddress(timeHost,
	...
	cfg.HctimeHost = "https://" + cfg.HctimeHost

	if len(cfg.HctimeCert) != 0 && !util.FileExists(cfg.HctimeCert) {
		cfg.HctimeCert = cleanAndExpandPath(cfg.HctimeCert)
		path := filepath.Join(cfg.HomeDir, cfg.HctimeCert)
		if !util.FileExists(path) {
			str := "%s: hctimecert " + cfg.HctimeCert + " and " +
				path + " don't exist"
			err := fmt.Errorf(str, funcName)
			fmt.Fprintln(os.Stderr, err)
			return nil, nil, err
