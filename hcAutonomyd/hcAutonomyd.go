...
// hcAutonomy application context.
type hcAutonomy struct {
...
func (p *hcAutonomy) convertBackendRecord(br backend.Record) v1.Record {
...
func (p *hcAutonomy) respondWithUserError(w http.ResponseWriter,
...
func (p *hcAutonomy) respondWithServerError(w http.ResponseWriter, errorCode int64) {
...
func (p *hcAutonomy) getIdentity(w http.ResponseWriter, r *http.Request) {
	var t v1.Identity
...
func (p *hcAutonomy) newRecord(w http.ResponseWriter, r *http.Request) {
	var t v1.NewRecord
	decoder := json.NewDecoder(r.Body)
	if err := decoder.Decode(&t); err != nil {
		p.respondWithUserError(w, v1.ErrorStatusInvalidRequestPayload, nil)
...
func (p *hcAutonomy) updateRecord(w http.ResponseWriter, r *http.Request, vetted bool) {
	cmd := "unvetted"
	if vetted {
		cmd = "vetted"
	}
...
func (p *hcAutonomy) updateUnvetted(w http.ResponseWriter, r *http.Request) {
	p.updateRecord(w, r, false)
}

func (p *hcAutonomy) updateVetted(w http.ResponseWriter, r *http.Request) {
	p.updateRecord(w, r, true)
}

func (p *hcAutonomy) getUnvetted(w http.ResponseWriter, r *http.Request) {
...
func (p *hcAutonomy) getVetted(w http.ResponseWriter, r *http.Request) {
	var t v1.GetVetted
	decoder := json.NewDecoder(r.Body)
	if err := decoder.Decode(&t); err != nil {
		p.respondWithUserError(w, v1.ErrorStatusInvalidRequestPayload, nil)
...
func (p *hcAutonomy) inventory(w http.ResponseWriter, r *http.Request) {
	var i v1.Inventory
	decoder := json.NewDecoder(r.Body)
...
func (p *hcAutonomy) check(user, pass string) bool {
...
func (p *hcAutonomy) auth(fn http.HandlerFunc) http.HandlerFunc {
...
func (p *hcAutonomy) setVettedStatus(w http.ResponseWriter, r *http.Request) {
...
func (p *hcAutonomy) setUnvettedStatus(w http.ResponseWriter, r *http.Request) {
...
func (p *hcAutonomy) updateVettedMetadata(w http.ResponseWriter, r *http.Request) {
...
func (p *hcAutonomy) pluginInventory(w http.ResponseWriter, r *http.Request) {
...
func (p *hcAutonomy) pluginCommand(w http.ResponseWriter, r *http.Request) {
...
func (p *hcAutonomy) addRoute(method string, route string, handler http.HandlerFunc, perm permission) {
...
func _main() error {
	...
	if !util.FileExists(loadedCfg.HTTPSKey) &&
		...
		err := util.GenCertPair(elliptic.P521(), "hcAutonomyd",
	...
	if len(loadedCfg.HctimeCert) != 0 {
		var certPool *x509.CertPool
		if !util.FileExists(loadedCfg.HctimeCert) {
			return fmt.Errorf("unable to find hctime cert %v",
				loadedCfg.HctimeCert)
		}
		hctimeCert, err := ioutil.ReadFile(loadedCfg.HctimeCert)
		if err != nil {
			return fmt.Errorf("unable to read hctime cert %v: %v",
				loadedCfg.HctimeCert, err)
		}
		certPool = x509.NewCertPool()
		if !certPool.AppendCertsFromPEM(hctimeCert) {
	...
	b, err := gitbe.New(activeNetParams.Params, loadedCfg.DataDir,
		loadedCfg.HctimeHost, "", p.identity, loadedCfg.GitTrace)
	if err != nil {
		return err
	}
	p.backend = b
