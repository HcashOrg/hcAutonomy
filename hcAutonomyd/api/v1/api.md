# hcAutonomyd API Specification

# v1

This document describes the REST API provided by a `hcAutonomyd` daemon.

`hcAutonomyd` provides a time-ordered cryptographically-accountable digital
...
Obtain `hcAutonomyd` Ed25519 public key. For more information go to
https://ed25519.cr.yp.to/

All `hcAutonomyd` commands are initiated by clients and shall contain a challenge
...
| | Type | Description |
|-|-|-|
| id | uint64 | ID of the metadata stream. Currently hcAutonomyd supports 16 (0..15) streams per record. |
