...
# run_admin_routes tests the hcAutonomywww api routes that require admin privileges
run_admin_routes() {
  echo "Admin - Login"
...
# run_vote_routes tests the hcAutonomywww api routes that handle proposal voting
run_vote_routes() {
  echo "Vote - Login"
  login=`$cmd login $admin_email $admin_password`
  check_error "$login"
...
print_usage() { 
  echo "Usage:
  hcAutonomywww_refclient.sh [options] [ -e admin_email ] [ -p admin_password ]
...
main() {
  ...
  while getopts 'e:hjo:p:s:v' flag; do
    case "${flag}" in
      ...
      s) cmd="hcAutonomywwwcli -j --host=${OPTARG}" ;;
      v) vote="true" ;;
      *) print_usage
         exit 1 ;;
    esac
