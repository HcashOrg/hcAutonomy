...
import (
	...
	wwwconfig "github.com/HcashOrg/hcAutonomy/hcAutonomywww/sharedconfig"
)
...
const (
	cli    = "hcAutonomywwwcli"
	dbutil = "hcAutonomywww_dbutil"
)

var (
	cfg            *config
	hcAutonomydCmd   *exec.Cmd
	hcAutonomywwwCmd *exec.Cmd
)
...
func createHcAutonomywwCmd(paywall bool) *exec.Cmd {
...
func startHcAutonomywww(paywall bool) error {
	fmt.Printf("Starting hcAutonomywww\n")
	hcAutonomywwwCmd = createHcAutonomywwCmd(paywall)
	out, _ := hcAutonomywwwCmd.StdoutPipe()
	if err := hcAutonomywwwCmd.Start(); err != nil {
		hcAutonomywwwCmd = nil
		return err
	}

	logFile, err := createLogFile(cfg.HcAutonomywwwLogFile)
	if err != nil {
		return err
	}
	...
	return getVersionFromHcAutonomywww()
}

func startHcAutonomyd() error {
	fmt.Printf("Starting hcAutonomyd\n")
	hcAutonomydCmd = executeCommand("hcAutonomyd", "--testnet")
	out, _ := hcAutonomydCmd.StdoutPipe()
	if err := hcAutonomydCmd.Start(); err != nil {
		hcAutonomydCmd = nil
		return err
	}

	logFile, err := createLogFile(cfg.HcAutonomydLogFile)
	if err != nil {
		return err
	}
...
func getVersionFromHcAutonomywww() error {
...
func createUserWithHcAutonomywww(email, username, password string) error {
...
func createPaidUsers() error {
	err := createUserWithHcAutonomywww(cfg.AdminEmail, cfg.AdminUser, cfg.AdminPass)
	if err != nil {
		return err
	}

	err = createUserWithHcAutonomywww(cfg.PaidEmail, cfg.PaidUser, cfg.PaidPass)
	if err != nil {
		return err
	}
	...
	if err = startHcAutonomyd(); err != nil {
		return err
	}

	if err = startHcAutonomywww(true); err != nil {
		return err
	}
...
func createUnpaidUsers() error {
	return createUserWithHcAutonomywww(cfg.UnpaidEmail, cfg.UnpaidUser, cfg.UnpaidPass)
}
...
func deleteExistingData() error {
	...
	// hcAutonomyd data dir
	hcAutonomydDataDir := filepath.Join(hcutil.AppDataDir("hcAutonomyd", false), "data")
	if err := os.RemoveAll(hcAutonomydDataDir); err != nil {
		return err
	}

	// hcAutonomywww data dir
	if err := os.RemoveAll(wwwconfig.DefaultDataDir); err != nil {
		return err
	}

	// hcAutonomywww cli dir
	cliHomeDir := filepath.Join(wwwconfig.DefaultHomeDir, "cli")
	return os.RemoveAll(cliHomeDir)
}

func stopHcAutonomyd() {
	if hcAutonomydCmd != nil {
		fmt.Printf("Stopping hcAutonomyd\n")
		hcAutonomydCmd.Process.Kill()
		hcAutonomydCmd = nil
	}
}

func stopHcAutonomywww() {
	if hcAutonomywwwCmd != nil {
		fmt.Printf("Stopping hcAutonomywww\n")
		hcAutonomywwwCmd.Process.Kill()
		hcAutonomywwwCmd = nil
	}
}
...
func _main() error {
	...
	stopHcAutonomywww()
	if err = startHcAutonomywww(false); err != nil {
		return err
	}
