...
import (
	...
	"github.com/HcashOrg/hcAutonomy/hcAutonomywww/sharedconfig"
...
const (
	...
	defaultLogFilename      = "hcAutonomywww.log"
...
// config defines the configuration options for hcAutonomywww.
//
...
type config struct {
	...
	RPCHost                  string `long:"rpchost" description:"Host for hcAutonomyd in this format"`
	RPCCert                  string `long:"rpccert" description:"File containing the https certificate file"`
	RPCIdentityFile          string `long:"rpcidentityfile" description:"Path to file containing the hcAutonomyd identity"`
	...
	SMTP                     *goemail.SMTP
	FetchIdentity            bool   `long:"fetchidentity" description:"Whether or not hcAutonomywww fetches the identity from hcAutonomyd."`
	WebServerAddress         string `long:"webserveraddress" description:"Address for the HcAutonomy web server; it should have this format: <scheme>://<host>[:<port>]"`
	...
	PaywallAmount            uint64 `long:"paywallamount" description:"Amount of HC (in atoms) required for a user to register or submit a proposal."`
...
// loadIdentity fetches an identity from hcAutonomyd if necessary.
func loadIdentity(cfg *config) error {
	// Set up the path to the hcAutonomyd identity file.
	...
	if _, err := os.Stat(cfg.RPCIdentityFile); os.IsNotExist(err) {
		return fmt.Errorf("you must load the identity from hcAutonomyd " +
