...
func (b *backend) loadVoteAuthorization(token, payload string) error {
	f := strings.NewReader(payload)
	d := json.NewDecoder(f)
	var avr HcashOrgplugin.AuthorizeVoteReply
...
func (b *backend) loadVoting(token, payload string) error {
	f := strings.NewReader(payload)
	d := json.NewDecoder(f)
	var md HcashOrgplugin.StartVoteReply
	if err := d.Decode(&md); err == io.EOF {
		return nil
	} else if err != nil {
		return err
	...
	p := b.inventory[token]
	p.voting = convertStartVoteReplyFromHcplugin(md)
	return nil
}
...
func (b *backend) loadVoteBits(token, payload string) error {
	f := strings.NewReader(payload)
	d := json.NewDecoder(f)
	var md HcashOrgplugin.StartVote
	if err := d.Decode(&md); err == io.EOF {
		return nil
	} else if err != nil {
		return err
	...
	p := b.inventory[token]
	p.votebits = convertStartVoteFromHcplugin(md)
	return nil
}

// loadComments calls out to the HcashOrg plugin to obtain all comments.
//
// This function must be called WITH the mutex held.
...
func (b *backend) loadComments(t string) error {
	...
	payload, err := HcashOrgplugin.EncodeGetComments(HcashOrgplugin.GetComments{
		Token: t,
	})
	if err != nil {
		return err
	...
	pc := pd.PluginCommand{
		...
		Command:   HcashOrgplugin.CmdGetComments,
		CommandID: HcashOrgplugin.CmdGetComments,
	...
	// Decode plugin reply
	gcr, err := HcashOrgplugin.DecodeGetCommentsReply([]byte(reply.Payload))
	if err != nil {
		return err
	}
	...
	for _, v := range gcr.Comments {
		c := b._convertHcCommentToWWWComment(v)
...
// loadComments calls out to the HcashOrg plugin to obtain all comments likes
// of a given proposal
//
// This function must be called WITH the mutex held.
func (b *backend) _loadCommentsLikes(token string) error {
	...
	payload, err := HcashOrgplugin.EncodeGetProposalCommentsLikes(
		HcashOrgplugin.GetProposalCommentsLikes{
			Token: token,
		})
	if err != nil {
		return err
	...
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        HcashOrgplugin.ID,
		Command:   HcashOrgplugin.CmdProposalCommentsLikes,
		CommandID: HcashOrgplugin.CmdProposalCommentsLikes,
		Payload:   string(payload),
	}
	...
	// Decode plugin reply
	gpclr, err := HcashOrgplugin.DecodeGetProposalCommentsLikesReply(
		[]byte(reply.Payload))
	if err != nil {
		return err
	}
	...
	for _, v := range gpclr.CommentsLikes {
		lc := convertHcLikeCommentToWWWLikeComment(v)
...
func (b *backend) loadRecordMetadata(v pd.Record) {
	...
	for _, m := range v.Metadata {
		...
		case HcashOrgplugin.MDStreamAuthorizeVote:
		...
		case HcashOrgplugin.MDStreamVoteBits:
		...
		case HcashOrgplugin.MDStreamVoteSnapshot:
