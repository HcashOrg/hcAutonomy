# hcAutonomywww API Specification

# v1

This document describes the REST API provided by a `hcAutonomywww` server.  The
`hcAutonomywww` server is the web server backend and it interacts with a JSON REST
...
| | Type | Description |
|-|-|-|
| errorcode | number | An error code that can be used to track down the internal server error that occurred; it should be reported to HcAutonomy administrators. |
...
Create a new user on the hcAutonomywww server.
...
| paywallamount | Int64 | The amount of HC (in atoms) to send to `paywalladdress`.  If the user has already paid, this field will be empty or not present. |
...
hcAutonomywww polls the paywall address until the paywall is either paid or it
...
Submit a new proposal to the hcAutonomywww server.
...
| publickey | string | Public key from the client side, sent to hcAutonomywww for verification | Yes |
...
Edit an existent proposal into the hcAutonomywww server.
The proposal name is derived from the first line of the markdown file - index.md.
...
| publickey | string | Public key from the client side, sent to hcAutonomywww for verification |
...
| receipt | string | HcAutonomyd signature of the client signature |
...
forwarded as-is to the hcAutonomy daemon.
...
| newuserpaywallamount | int64 | The amount of HC (in atoms) to send to `newuserpaywalladdress`.  If the user has already paid, this field will be empty or not present. |
...
| digest | string | Digest is a SHA256 digest of the payload. The digest shall be verified by hcAutonomyd. |
...
| txid | string | The txID of the Hc transaction that paid for this credit. |
