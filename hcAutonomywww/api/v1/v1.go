...
// PDError is emitted when an HTTP error response is returned from HcAutonomyd
...
func (e PDError) Error() string {
	return fmt.Sprintf("error from hcAutonomyd: %v %v", e.HTTPCode,
		e.ErrorReply.ErrorCode)
}

// PDErrorReply is an error reply returned from HcAutonomyd whenever an
// error occurs.
...
// provided transaction on the Hc blockchain and verify that it
...
// the file upload restrictions set for HcAutonomy.
