...
import (
	...
	"github.com/HcashOrg/hcAutonomy/hcAutonomywww/database/localdb"
...
const (
	...
	// Note that 14 is in use by the HcashOrg plugin
	// Note that 15 is in use by the HcashOrg plugin
...
// hcAutonomywww backend construct
type backend struct {
	...
	client          *http.Client // hcAutonomyd client
...
// remoteInventory fetches the entire inventory of proposals from hcAutonomyd.
...
// loadInventory calls the hcAutonomyd RPC call to load the current inventory.
...
// LoadInventory fetches the entire inventory of proposals from hcAutonomyd and
...
// ProcessNewProposal tries to submit a new proposal to hcAutonomyd.
...
func (b *backend) ProcessSetProposalStatus(sps www.SetProposalStatus, user *database.User) (*www.SetProposalStatusReply, error) {
	...
	// hcAutonomyd returns the record without the files
...
// ProcessProposalDetails tries to fetch the full details of a proposal from hcAutonomyd.
...
func (b *backend) ProcessComment(c www.NewComment, user *database.User) (*www.NewCommentReply, error) {
	...
	ndc := convertWWWNewCommentToHcNewComment(c)
	payload, err := HcashOrgplugin.EncodeNewComment(ndc)
	if err != nil {
		return nil, err
	}
	...
	pc := pd.PluginCommand{
		...
		Command:   HcashOrgplugin.CmdNewComment,
		CommandID: HcashOrgplugin.CmdNewComment,
	...
	// Decode plugin reply
	ncr, err := HcashOrgplugin.DecodeNewCommentReply([]byte(reply.Payload))
	if err != nil {
		return nil, err
	}

	// Note this call takes the read lock.
	ncrWWW := b.convertHcNewCommentReplyToWWWNewCommentReply(*ncr)
...
func (b *backend) ProcessLikeComment(lc www.LikeComment, user *database.User) (*www.LikeCommentReply, error) {
	...
	nlc := convertWWWLikeCommentToHcLikeComment(lc)
	payload, err := HcashOrgplugin.EncodeLikeComment(nlc)
	if err != nil {
		return nil, err
	}
	...
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        HcashOrgplugin.ID,
		Command:   HcashOrgplugin.CmdLikeComment,
		CommandID: HcashOrgplugin.CmdLikeComment,
		Payload:   string(payload),
	}
	...
	// Decode plugin reply
	lcr, err := HcashOrgplugin.DecodeLikeCommentReply([]byte(reply.Payload))
	if err != nil {
		return nil, err
	}
...
func (b *backend) ProcessCensorComment(cc www.CensorComment, user *database.User) (*www.CensorCommentReply, error) {
	...
	dcc := convertWWWCensorCommentToHcCensorComment(cc)
	payload, err := HcashOrgplugin.EncodeCensorComment(dcc)
	if err != nil {
	...
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        HcashOrgplugin.ID,
		Command:   HcashOrgplugin.CmdCensorComment,
		CommandID: HcashOrgplugin.CmdCensorComment,
		Payload:   string(payload),
	}
	...
	// Decode plugin reply.
	ccr, err := HcashOrgplugin.DecodeCensorCommentReply([]byte(reply.Payload))
	if err != nil {
		return nil, fmt.Errorf("DecodeCensorCommentReply: %v", err)
	}
	ccrWWW := convertHcCensorCommentReplyToWWWCensorCommentReply(*ccr)
...
func (b *backend) ProcessCastVotes(ballot *www.Ballot) (*www.BallotReply, error) {
	...
	payload, err := HcashOrgplugin.EncodeBallot(convertBallotFromWWW(*ballot))
	if err != nil {
		return nil, err
	}
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        HcashOrgplugin.ID,
		Command:   HcashOrgplugin.CmdBallot,
		CommandID: HcashOrgplugin.CmdBallot,
		Payload:   string(payload),
	}
	...
	// Decode plugin reply
	br, err := HcashOrgplugin.DecodeBallotReply([]byte(reply.Payload))
	if err != nil {
		return nil, err
	}
	brr := convertBallotReplyFromHcPlugin(*br)
	return &brr, nil
}

// ProcessAuthorizeVote sends the authorizevote command to HcashOrg plugin to
...
func (b *backend) ProcessAuthorizeVote(av www.AuthorizeVote, user *database.User) (*www.AuthorizeVoteReply, error) {
	...
	payload, err := HcashOrgplugin.EncodeAuthorizeVote(dav)
	if err != nil {
	...
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        HcashOrgplugin.ID,
		Command:   HcashOrgplugin.CmdAuthorizeVote,
		CommandID: HcashOrgplugin.CmdAuthorizeVote + " " + av.Token,
		Payload:   string(payload),
	}
	...
	// Decode plugin reply
	avr, err := HcashOrgplugin.DecodeAuthorizeVoteReply([]byte(reply.Payload))
	if err != nil {
		return nil, fmt.Errorf("DecodeAuthorizeVoteReply: %v", err)
	}
	avrWWW := convertAuthorizeVoteReplyFromHcplugin(*avr)
...
func (b *backend) ProcessStartVote(sv www.StartVote, user *database.User) (*www.StartVoteReply, error) {
	...
	payload, err := HcashOrgplugin.EncodeStartVote(dsv)
	if err != nil {
		return nil, err
	}
	...
	// Tell HcashOrg plugin to start voting
	challenge, err := util.Random(pd.ChallengeSize)
	if err != nil {
		return nil, err
	}
	...
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        HcashOrgplugin.ID,
		Command:   HcashOrgplugin.CmdStartVote,
		CommandID: HcashOrgplugin.CmdStartVote + " " + sv.Vote.Token,
		Payload:   string(payload),
	}
	...
	vr, err := HcashOrgplugin.DecodeStartVoteReply([]byte(reply.Payload))
	if err != nil {
		return nil, err
	}

	voting := convertStartVoteReplyFromHcplugin(*vr)
...
func (b *backend) ProcessVoteResults(token string) (*www.VoteResultsReply, error) {
	...
	wvrr := convertVoteResultsReplyFromHcplugin(*vrr, ir)
	return &wvrr, nil
}
...
func (b *backend) ProcessGetAllVoteStatus() (*www.GetAllVoteStatusReply, error) {
	...
	for _, i := range b.inventory {
		...
		vsr := www.VoteStatusReply{
			...
			OptionsResult:      convertVoteResultsFromHcplugin(*vrr),
...
// ProcessEditProposal attempts to edit a proposal on hcAutonomyd
...
// ProcessPolicy returns the details of HcAutonomy's restrictions on file uploads.
...
func (b *backend) getBestBlock() (uint64, error) {
	...
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        HcashOrgplugin.ID,
		Command:   HcashOrgplugin.CmdBestBlock,
		CommandID: HcashOrgplugin.CmdBestBlock,
		Payload:   "",
	}
...
func (b *backend) getVoteResultsFromPlugin(token string) (*HcashOrgplugin.VoteResultsReply, error) {

	payload, err := HcashOrgplugin.EncodeVoteResults(HcashOrgplugin.VoteResults{
		Token: token,
	})
	if err != nil {
		return nil, err
	...
	pc := pd.PluginCommand{
		Challenge: hex.EncodeToString(challenge),
		ID:        HcashOrgplugin.ID,
		Command:   HcashOrgplugin.CmdProposalVotes,
		CommandID: HcashOrgplugin.CmdProposalVotes + " " + token,
		Payload:   string(payload),
	}
	...
	vrr, err := HcashOrgplugin.DecodeVoteResultsReply([]byte(reply.Payload))
	if err != nil {
		return nil, err
	}
